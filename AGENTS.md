...
- For discovery, if `pw bind` receives a different bind UUID for the same canonical domain and schema, raise an error instead of replacing the stored bind, append an `ALERT` entry to `~/.pollyweb/binds.log` including the triggering script path, and attempt a local OS notification so concurrent churn is obvious.
