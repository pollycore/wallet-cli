...
If the wallet files already exist, `pw onboard` makes an idempotent `Onboard@Notifier` call with the same public key and expects the same wallet id back. If the notifier returns a different wallet id for the same key pair, the command fails with a drift error instead of rewriting the stored config.
