...
When a Notifier is configured and the sender is a bound wallet, `pw echo` automatically sets up an async response channel before sending — the same channel flow as `pw msg`. If the target domain replies with `Meta.Code: 202`, the CLI waits for the final signed response to arrive over the WebSocket before running verification.
