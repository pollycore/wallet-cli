...
The file must contain an `Outbound` object and may also contain `Inbound` and `Wait` fields:
...
If the client-side wallet transport times out before any reply arrives, `pw test` now says that explicitly as a client timeout, reports the configured client timeout budget, and includes the measured send duration. When the fixture also used `Wait`, the timeout message includes that pre-send wait separately so it is clear the timeout budget applies to the send/read phase rather than the fixture delay. Because no response was received in that case, there is no server timing available to report.

On success, `pw test` keeps the output concise. For non-interactive output and single-fixture runs, it prints one short line per passing fixture in the form `✅ Passed: <filename-without-extension> (<total-ms> ms, <latency>% latency)`. For interactive grouped runs that already show the completed hierarchy in the live parallel status tree, the CLI should not print the same settled results again afterward as a duplicate final block. When the synchronous response is wrapped and includes `Response.Meta.TotalMs`, `pw test` uses that server-reported total as a timing hint for the displayed total duration and latency share.

If an HTTP request returns a non-200 status, `pw test` normally fails the fixture with that HTTP error. The one exception is when the fixture explicitly expects the same status in `Inbound.Meta.Code`: in that case, the CLI treats the HTTP error body as the response payload and runs the normal `Inbound` subset assertion against it, so expected error responses such as `404` can count as successful tests.
