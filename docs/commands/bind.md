...
Each successful `pw bind` write also appends a wallet-managed audit entry to `~/.pollyweb/binds.log`. Unexpected bind changes append an `ALERT` entry there as well, and on macOS the CLI also attempts to raise a local notification so concurrent test runs are easier to spot.
