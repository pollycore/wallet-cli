...
Shell mode requires a stored bind for the target domain and will ask you to run `pw bind <domain>` first if none exists. The CLI prints the response body and waits for the next command. Empty commands are ignored. Shell mode exits when you press `Ctrl+D`, press `Ctrl+C`, or when a request error occurs.
